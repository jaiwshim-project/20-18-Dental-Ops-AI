  <script>
document.getElementById('app').insertAdjacentHTML('afterbegin', renderSidebar('review-wide'));
...
// ============================================================
// 리뷰 모드 — URL 파라미터 ?patient=<id>&session=<sid> 자동 진입
// ============================================================
(async function autoLoadReviewMode() {
  const sel = document.getElementById('patientSelect');

  // 세션 레이블: "#3회차 · 26/04/20 14:30 · 12턴 · 15분"
  function formatSessionLabel(s, idx) {
    const d = new Date(s.created_at || s.at);
    const datePart = d.toLocaleDateString('ko-KR', { year: '2-digit', month: '2-digit', day: '2-digit' });
    const timePart = d.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' });
    const m = s.metadata || {};
    const turns = m.turns_count || m.turnsCount || 0;
    const dur = m.duration_sec || m.durationSec || 0;
    const min = dur > 0 ? Math.max(1, Math.round(dur / 60)) + '분' : '';
    const parts = [`#${idx}회차`, `${datePart} ${timePart}`];
    if (turns) parts.push(turns + '턴');
    if (min) parts.push(min);
    return parts.join(' · ');
  }

  // 환자 선택 → 세션 목록 조회 + 드롭다운 채우기 + 대상 세션 로드
  async function loadSessionsForPatient(preferSessionId) {
    const pid = sel.value;
    const wrap = document.getElementById('sessionSelectWrap');
    const ssel = document.getElementById('sessionSelect');
    if (wrap) wrap.style.display = 'none';
    if (ssel) ssel.innerHTML = '<option value="">-- 회차 선택 --</option>';
    if (!pid) return;

    const sessions = await getPatientSessions(pid);
    if (!sessions.length) {
      showToast('이 환자의 저장된 상담이 없습니다', 'info');
      refreshPatientHistoryBadge();
      return;
    }

    // 세션은 최신 순(created_at desc). 회차 번호는 최신=가장 큰 숫자로 매김
    sessions.forEach((s, idx) => {
      const reverseIdx = sessions.length - idx;
      const opt = document.createElement('option');
      opt.value = s.id;
      opt.textContent = formatSessionLabel(s, reverseIdx);
      ssel.appendChild(opt);
    });

    const targetId = preferSessionId || sessions[0].id;
    ssel.value = targetId;
    if (sessions.length > 1 && wrap) wrap.style.display = 'block';
    await loadPastSession(targetId);

    if (sessions.length > 1) {
      showToast(`${sessions.length}개 회차 · 다른 회차는 드롭다운에서 선택`, 'info');
    }
  }

  // 세션 드롭다운 change 핸들러 (onclick용 전역)
  window.onSessionSelectChange = async function() {
    const ssel = document.getElementById('sessionSelect');
    const sid = ssel?.value;
    if (!sid) return;
    await loadPastSession(sid);
  };

  sel.addEventListener('change', () => loadSessionsForPatient());

  // URL 파라미터 자동 진입
  const urlParams = new URLSearchParams(window.location.search);
  const pid = urlParams.get('patient');

  // populatePatients가 끝나기를 기다림 (최대 5초)
  const waitStart = Date.now();
  while (patientCache.length === 0 && Date.now() - waitStart < 5000) {
    await new Promise(r => setTimeout(r, 200));
  }

  if (!pid) {
    showToast('환자를 선택하면 회차 목록이 표시됩니다', 'info');
    return;
  }

  if (!patientCache.find(p => p.id === pid)) {
    showToast('해당 환자를 찾을 수 없습니다', 'warning');
    return;
  }
  sel.value = pid;
  await loadSessionsForPatient(urlParams.get('session'));
})();
...
async function refreshPatientHistoryBadge() {
  ...
  el.innerHTML = `<div class="patient-hist-badge">
    <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px;">
      ...
      <button onclick="openPastSessionsModal()" class="btn btn-sm btn-primary" style="flex-shrink:0; font-size:0.72rem; padding:5px 10px;">📂 불러오기</button>
    </div>
  </div>`;
}
...
function openPastSessionsModal() {
  ...
  // 모달 대신 독립 페이지로 이동
  window.location.href = 'consult_review.html?patient=' + encodeURIComponent(patient.id);
}
...
async function runCoach() {
  ...
  area.style.display = 'block';
  area.innerHTML = '<span class="spinner-inline"></span> 환자 의도 분석 + 스킬·하네스로 코칭 스크립트를 생성 중입니다...';
  coachArea.innerHTML = '';
  badge.innerHTML = '';
  ...
  try {
    ...
    const utterances = hasTurns
      ? turns.map(t => ({ text: t.text, at: t.at }))
      : [{ text: manualQ, at: Date.now() }];

    const coachRes = await ConsultEngine.coachReply({
      patient,
      utterances,
      history: hist
    });
